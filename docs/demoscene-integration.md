...
**Status:** **integrated** — this is now the contract reference, not a to-do list
**Package:** `@site/demoscene` — `packages/demoscene/`
**Host:** `apps/guitar-codex` (the stand-in host is `apps/demoscene-dev-host`)
**Integration surface:** one lazy-loaded React component and one string prop.

> ### Where it is wired up, in this repository
>
> The demoscene opens from the **`I` button in the top bar**. The seam is three
> files, and §2's install steps are already done:
>
> ```text
> apps/guitar-codex/src/components/about/
>   LazyAboutDemoscene.ts   the dynamic import — the async boundary
>   AboutDemoscene.tsx      <Demoscene open locale onClose /> and nothing else
>   index.ts                barrel: the lazy entry only, never AboutDemoscene
> ```
>
> plus `aboutOpen` + the `I` button in `App.tsx`, and `FEATURES.demoscene` in
> `apps/guitar-codex/src/config.ts` to turn it off. The mount is the **modal**
> shape of §3.1, not the route of §3.2: the demoscene is an interlude, not a
> place, so it deliberately has no URL of its own.
>
> Read §5 (the contract), §6 (ownership), §7 (teardown) and §11
> (anti-patterns) before changing any of it.
...
| `apps/demoscene-dev-host/` | no | A throwaway React app standing in for `apps/encyclopedia`. Exists so the demoscene can be run and reviewed without the encyclopedia. Its copy, its language picker and its error boundary are *host* concerns and are examples, not shipped code. |
| `apps/demoscene-dev-host/src/{sketches,portraits}.ts` | no | Local plate viewers for tuning the pencil studies. They deep-import demoscene internals; see §34.3 — do not copy that. |
| `apps/demoscene-dev-host` standalone build | no | A single-file IIFE for dropping the demoscene into a plain HTML page. Kept as an escape hatch (§9 below), not part of the React path. |
