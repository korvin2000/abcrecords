...
>
> **Stand:** 2026-08-31 · **Package-Version:** 0.2.0 · **Status: eingebunden**

> ### Wo es in diesem Repository verdrahtet ist
>
> Das Gästebuch öffnet der Eintrag **„VI · Gästebuch"** im Seitenfuß, unter der
> reservierten Route `#/guestbook`. Die Nahtstelle sind vier Dateien; die
> Installationsschritte aus Abschnitt 3 und 4 sind erledigt:
>
> ```text
> apps/guitar-codex/src/components/guestbook/
>   LazyGuestbookOverlay.ts  der dynamische Import — die Lazy-Grenze
>   GuestbookOverlay.tsx     der Codex-Rahmen; darin <Guestbook …/>
>   route.ts                 der reservierte Slug "guestbook"
>   index.ts                 Barrel: nur der Lazy-Einstieg
> ```
>
> Dazu: `FEATURES.guestbook` und `GUESTBOOK.apiBaseUrl` in
> `apps/guitar-codex/src/config.ts`, der Dev-Proxy in
> `apps/guitar-codex/vite.config.ts` (Abschnitt 9), und die `--gb-*`-Tokens in
> `apps/guitar-codex/src/styles/guestbook.css` (Abschnitt 10) — dort steht die
> Klasse **doppelt**, weil das Stylesheet des Packages im Async-Chunk *nach*
> `index.css` geladen wird und den Spezifitäts-Gleichstand sonst gewinnt.
>
> Übergeben werden `locale`, `languageSwitcher="never"` (die Host-Anwendung hat
> einen eigenen Umschalter), `theme="light"`, `header={false}` und
> `config.apiBaseUrl`. `routing` bleibt auf `memory`: der Hash gehört dem
> Katalog.
...
(deren `migration-plan.md` und `deployment.md` liegen im Guestbook-Repository,
nicht hier).
...
- [../.claude-memory/17-workspace-and-features.md](../.claude-memory/17-workspace-and-features.md) — der Workspace und beide Nahtstellen auf einen Blick
- [../packages/guestbook/README.md](../packages/guestbook/README.md) — Package-Innenleben und Verzeichnisaufbau
- [../packages/guestbook/docs/visitor-features.md](../packages/guestbook/docs/visitor-features.md) — was das Gästebuch fachlich kann
- [../packages/guestbook/docs/legacy-architecture.md](../packages/guestbook/docs/legacy-architecture.md) — die Alt-Anwendung, aus der es hervorging

> `migration-plan.md`, `deployment.md` und `database-schema.md` beschreiben die
> PHP-API und die Datenbank. Sie liegen im **Guestbook-Repository** (`gbook`),
> nicht hier — hierher kam nur das Frontend-Package.
