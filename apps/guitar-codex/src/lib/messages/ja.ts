import type { Message } from "./types";
import type { MsgKey } from "./ru";

export const ja: Record<MsgKey, Message> = {
  "app.title": "ギターの写本",
  "app.volume": "✦ 第一巻 · クラシックギター ✦",
  "app.subtitle": "音楽家たちの百科事典 — その名と運命と伝説を、黄ばんだ頁に記す。",
  "app.brand": "写本",
  "app.footer": "✦ 金で装丁された書 · 記された名はその頁を開く · ← → キーで記録をめくる ✦",
  "app.loadError": "年代記を開けませんでした。巻物 index.json に届きません。",
  "app.retry": "再試行",
  "app.crash": "写本が途切れました — この頁を描けませんでした。",
  "app.crashReload": "写本を開き直す",
  "app.crashCodex": "この記録は開けませんでした。頁を再読み込みしてお試しください。",

  "footer.kicker": "書庫 · 案内 · 便り",
  "footer.title": "プロジェクトの各章",
  "footer.navLabel": "プロジェクト各章の案内",
  "footer.about": "プロジェクトについて",
  "footer.sources": "出典",
  "footer.literature": "文献",
  "footer.links": "リンク",
  "footer.news": "新着",
  "footer.guestbook": "芳名帳",
  "footer.search": "検索",
  "footer.email": "メール",
  "footer.audioMap": "音の地図",
  "footer.placeholderTitle": "準備中の章",
  "footer.placeholder": "「{section}」の章は後日追加されます。",
  "footer.usageNotice": "資料を利用する際は、プロジェクト「ギタリストと作曲家」へのリンクが必須です",
  "footer.rights": "無断転載を禁ず © タヴロフスキー V.V.・A.V.・S.V., 2001–2026",

  "search.placeholder": "名前で探す…",
  "search.clear": "検索を消去",
  "search.count": { other: "年代記に {n} 件の記録" },
  "search.countFiltered": "{total} 件中 {n} 件が見つかりました",
  "search.empty.title": "その名に応える者はいない。",
  "search.empty.hint": "写本にその名の記録はありません。別の名を試すか、絞り込みを解いてください。",
  "search.otherLangs": "他の言語で見つかった記録",
  "grid.more": "さらに表示",
  "grid.shown": "{total} 件中 {n} 件を表示",
  "facet.country.group": "国で絞り込む",

  "search.advanced": "検索を絞り込む",
  "search.advanced.hide": "絞り込みを隠す",
  "search.advanced.title": "詳細検索",
  "search.advanced.reset": "絞り込みを解除",
  "search.refinements": { other: "絞り込み {n} 件" },
  "search.scope": "記録の言語",
  "search.scope.all": "すべて",
  "search.scope.current": "現在",
  "search.gender": "性別",
  "search.gender.any": "指定しない",
  "search.forename": "名",
  "search.surname": "姓",
  "search.country.any": "すべての国",
  "search.born": "生年",
  "search.died": "没年",
  "search.yearFrom": "から",
  "search.yearTo": "まで",
  "search.dossier.hint": "名・姓・年は各記録の名簿から読み取ります。",
  "search.dossier.progress": "名簿を照合中 — {n}%",

  "facet.type": "生業",
  "facet.country": "国",

  "type.guitarist": "ギタリスト",
  "type.musician": "音楽家",
  "type.composer": "作曲家",
  "type.conductor": "指揮者",
  "type.luthier": "弦楽器職人",
  "type.magazine": "雑誌",

  "card.open": "記録を開く:{name}",
  "card.foreignHint": "利用可能な言語:{langs}",

  "herald.label.born": "この日 · 誕生",
  "herald.label.died": "この日 · 追悼",
  "herald.label.quote": "語録より",
  "herald.born.m": "本日でちょうど{years}前、{name}が生まれました",
  "herald.born.f": "本日でちょうど{years}前、{name}が生まれました",
  "herald.born.x": "本日でちょうど{years}前 — {name}の誕生日",
  "herald.died.m": "本日でちょうど{years}前、{name}が世を去りました",
  "herald.died.f": "本日でちょうど{years}前、{name}が世を去りました",
  "herald.died.x": "本日でちょうど{years}前 — {name}を追悼して",

  "about.open": "この写本について",

  "guestbook.close": "✕ 芳名帳を閉じる",
  "guestbook.error": "芳名帳を開けませんでした。ページを再読み込みしてお試しください。",

  "codex.entry": "❖ 写本の記録 ❖",
  "codex.close": "✕ 写本を閉じる",
  "codex.end": "❦ 記録の終わり · 頁をめくって戻る ❦",
  "codex.prev": "前の記録",
  "codex.next": "次の記録",
  "codex.notFound": "この記録の頁は、まだ写本に記されていません。",

  "tabs.biography": "年代記",
  "tabs.gallery": "メディア",
  "tabs.documents": "文書",
  "tabs.lore": "属性",

  "gallery.photos": "肖像と写真",
  "gallery.music": "音楽",
  "gallery.theme": "人物のテーマ",
  "gallery.themePlay": "▶︎ テーマを聴く",
  "gallery.themeStop": "■ 停止",
  "gallery.themeHint": "旋律は人物の名から f = f₀·2^(n/12) の式で生まれます",
  "gallery.empty": "この記録の画廊はまだ空です。",
  "gallery.close": "画像を閉じる",
  "audio.unavailable": "この録音は書庫に眠っており、再生できません",
  "audio.play": "再生",
  "audio.pause": "一時停止",
  "audio.download": "ダウンロード",
  "audio.noCodec": "この形式はブラウザーで再生できません — ダウンロードしてお聴きください",

  "viewer.zoomIn": "拡大",
  "viewer.zoomOut": "縮小",
  "viewer.rotateLeft": "左に回転",
  "viewer.rotateRight": "右に回転",
  "viewer.fit": "画面に合わせる",
  "viewer.actualSize": "実寸大",
  "viewer.fitWidth": "幅に合わせる",
  "viewer.download": "ダウンロード",
  "viewer.close": "閉じる",
  "viewer.error": "画像を読み込めませんでした",

  "pdf.loading": "文書を開いています…",
  "pdf.error": "文書を開けませんでした",
  "pdf.page": "ページ",
  "pdf.pageOf": "／ {total}",
  "pdf.prevPage": "前のページ",
  "pdf.nextPage": "次のページ",
  "pdf.fitWidth": "幅に合わせる",
  "pdf.fitPage": "ページ全体",
  "tab.title": "ASCIIタブ譜",
  "tab.loading": "タブ譜を読み込んでいます…",
  "tab.error": "タブ譜を読み込めませんでした",
  "tab.graphical": "グラフィック",
  "tab.raw": "原文",
  "tab.stop": "停止",
  "tab.approximate": "近似再生：リズムは文字間隔から推定し、不明な調弦は標準と仮定",
  "tab.system": "段 {number}",
  "tab.systemCount": "{count} 段",
  "tab.noSystems": "6弦の段を検出できませんでした。原文は表示できます。",
  "tab.diagnostics": "原文の曖昧な箇所",
  "tab.meter": "拍子",
  "tab.tuning": "チューニング",
  "tab.tuning.standard": "標準",
  "tab.tuning.drop-d": "ドロップD",
  "tab.tuning.unknown": "記載なし",
  "tab.tuning.conflicting": "記載が矛盾",
  "tab.encodingGuessed": "文字コードの宣言がないため、バイト列から推定しました。",

  "docs.source": "一次資料",
  "docs.external": "外部資料",
  "docs.archive": "書庫の参照",
  "docs.embedded": "写本に収録",
  "docs.empty": "この記録に巻物は添えられていません。",
  "docs.open": "開く",

  "lore.title": "身上書",
  "lore.identity": "人物",
  "lore.career": "生業と道のり",
  "lore.relations": "師と弟子",
  "lore.birthname": "出生名",
  "lore.gender": "性別",
  "lore.gender.m": "男",
  "lore.gender.f": "女",
  "lore.type": "生業",
  "lore.country": "国",
  "lore.birthplace": "出生地",
  "lore.deathplace": "没地",
  "lore.born": "生誕",
  "lore.died": "死没",
  "lore.age": "享年",
  "lore.years": { other: "{n} 歳" },
  "lore.activeYears": "活動期間",
  "lore.instruments": "楽器",
  "lore.genres": "ジャンル",
  "lore.bands": "楽団",
  "lore.jobs": "職務",
  "lore.teachers": "師",
  "lore.disciples": "弟子",
  "lore.relatives": "縁者",
  "lore.awards": "栄誉",
  "lore.ranking": "名声",
  "lore.noData": "この記録の身上書は、まだ編まれていません。",

  "bio.missing": "この記録の年代記は、まだ書かれていません。",
  "bio.archiveLink": "旧年代記からの書庫リンク",
  "bio.attachedDocument": "添付の巻物",

  "sound.on": "効果音:オン",
  "sound.off": "効果音:オフ",
  "ambient.on": "環境音:オン",
  "ambient.off": "環境音:オフ",
  "fx.on": "演出:オン",
  "fx.off": "演出:オフ",

  "lang.menu": "言語",
  "lang.title": "写本の言語",
  "lang.entry": "記録の言語",

  /* ---- the visitor counter (components/counter) ---- */

  "counter.label": "来訪者",
  "counter.open": "訪問統計",
  "counter.silent": "計数器は沈黙している — 集計を読み取れませんでした",

  "stats.title": "来訪者の記録",
  "stats.since": "{date} より記録 · {days}",
  "stats.dayCount": "{v}日",
  "stats.views": "{v}回の閲覧",
  "stats.close": "閉じる",
  "stats.retry": "再試行",
  "stats.loading": "読者を数えています…",
  "stats.error": "来訪者の記録を読み込めませんでした。",
  "stats.privacy": "クッキーも住所も用いず、集計のみ。",
  "stats.empty": "まだ記録がありません。",
  "stats.entryUnknown": "目録にありません",

  "stats.tile.uniques": "読者",
  "stats.tile.views": "開かれた頁",
  "stats.tile.visits": "訪問",
  "stats.tile.today": "本日",
  "stats.tile.online": "現在の閲覧",

  "stats.days.title": "過去三十日",
  "stats.days.hint": "七日で {d7} · 三十日で {d30}",
  "stats.hours.title": "本日、時間ごと",
  "stats.hours.hint": "最も多い時刻：{hour} · {n}",
  "stats.week.title": "曜日ごと",
  "stats.pages.title": "最も読まれた項目",
  "stats.langs.title": "言語別の読者",
  "stats.tech.title": "閲覧の道具",
  "stats.refs.title": "訪問の経路",
  "stats.refs.none": "すべての訪問がこの写本へ直に。",
  "stats.records.title": "余白の書き込み",

  "stats.tech.device": "機器",
  "stats.tech.browser": "ブラウザ",
  "stats.tech.os": "システム",
  "stats.device.desktop": "卓上機",
  "stats.device.mobile": "携帯",
  "stats.device.tablet": "タブレット",
  "stats.other": "その他",

  "stats.record.peakDay": "最も多い日",
  "stats.record.peakHour": "最も多い時刻",
  "stats.record.streak": "連続日数",
  "stats.record.perDay": "一日あたりの頁",
  "stats.record.perVisit": "一訪問あたりの頁",
  "stats.record.bots": "退けた巡回ロボット",
};
