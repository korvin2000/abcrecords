...
import { LazyAboutDemoscene, preloadAboutDemoscene } from "@/components/about";
import { GUESTBOOK_SLUG, LazyGuestbookOverlay } from "@/components/guestbook";
...
export default function App() {
  ...
  // The About demoscene (packages/demoscene). Deliberately *not* a route: it is
  // an interlude, not a place, and a reader who reloads should land back in the
  // catalogue rather than in the middle of a 96-second production.
  ...
  // The demoscene brings its own baroque score and its own AudioContext. Two
  // soundtracks at once is nobody's idea of ambience, so the catalogue's hushes
  // while it plays and returns when the reader does — without forgetting that
  // it was asked for.
  useEffect(() => {
    audio.setAmbient(sound && ambient && !aboutOpen);
  }, [sound, ambient, aboutOpen]);
  ...
  // Memoised on purpose: `DemosceneApp` rebuilds its production whenever its
  // options change by identity, and a fresh inline arrow every render would
  // rebuild it every render (docs/demoscene-integration.md §3.1).
  ...
  // `#/guestbook` is the one address in the catalogue's space that is spoken
  // for: it opens the visitors' book instead of an entry. Resolved *before*
  // `catalog.bySlug`, so a stray index row named "guestbook" cannot quietly
  // take the route back (see components/guestbook/route.ts).
  const guestbookOpen = FEATURES.guestbook && selectedSlug === GUESTBOOK_SLUG;
  const selectedRecord =
    selectedSlug && !guestbookOpen ? (catalog.bySlug.get(selectedSlug) ?? null) : null;

  // Single owner of the body scroll lock. Keyed on *whether* a full-screen
  // surface is open, never on which one: depending on the record would unlock
  // and re-lock the body on every ← →, and letting the page scrollbar back for
  // even one commit reflows the whole grid underneath the modal for nothing.
  // The guestbook shares the lock for the same reason.
  //
  // The demoscene is deliberately absent: it locks and restores the body
  // itself, recording whatever it found — including this "hidden" — as the
  // value to hand back, so the two nest correctly and neither strands the other
  // (docs/demoscene-integration.md §7).
  const overlayOpen = selectedRecord !== null || guestbookOpen;
  useEffect(() => {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* "VI · Guestbook" is openable without being a catalogue entry — it is
          the reserved route. Every other section still has to exist in the
          index before the footer will link to it rather than announce it. */}
      <SiteFooter
        canOpen={(slug) => (slug === GUESTBOOK_SLUG ? FEATURES.guestbook : catalog.bySlug.has(slug))}
      ...
      {/* The visitors' book. A feature package (packages/guestbook) behind its
          own async chunk, framed by the host — the overlay carries the boundary
          and the parchment; the package knows nothing about either. Mounted
          conditionally, so the chunk is fetched on the first VI and the panel's
          state is gone when it closes. */}
      <AnimatePresence>
        {guestbookOpen && (
          <Suspense fallback={null}>
            <LazyGuestbookOverlay key="guestbook" onClose={() => openEntry(null)} />
          </Suspense>
        )}
      </AnimatePresence>

      {/* The About demoscene. Mounted conditionally, never left mounted with
          `open={false}`: the chunk is fetched at first *render*, and it is the
          unmount that releases the rAF loop, the AudioContext and the body
          lock. Inside its chunk it renders `null` and mounts its own shadow
          root on `document.body`, so it costs this tree nothing.

          No fallback on either wrapper: the demoscene is a full-screen
          overlay, so a spinner behind nothing is worse than a beat of silence,
          and a chunk that will not load should leave the catalogue exactly as
          it was. */}
      {aboutOpen && (
        <ErrorBoundary label="demoscene" resetKey="about" fallback={null}>
          <Suspense fallback={null}>
            <LazyAboutDemoscene locale={lang} onClose={closeAbout} />
          </Suspense>
        </ErrorBoundary>
      )}
...
function CtrlButton({
  ...
  onClick: () => void;
  /** Warm whatever the click will need — hover and focus both count as intent
   *  (the same bargain `CharacterCard` strikes with the codex chunk). */
  onIntent?: () => void;
  active: boolean;
  title: string;
}) {
