...
interface Features {
  ...
  herald: boolean;
  /**
   * The visitors' book — the `@guitar-codex/guestbook` feature package, opened
   * from the footer's "VI · Guestbook" entry at `#/guestbook`. Off leaves the
   * entry as an inert placeholder, exactly as it was before the package
   * existed, and the chunk is never requested.
   */
  guestbook: boolean;
  /**
   * The About/credits demoscene — the `@site/demoscene` feature package, opened
   * from the `I` button in the top bar. Off removes the button.
   */
  demoscene: boolean;
}
...
interface GuestbookConfig {
   ...
   * straight through to the package as `config.apiBaseUrl`; no trailing slash.
   *
   * Site-root-relative, like `pdf` and the visitor counter: the API sits beside
   * the app on the same origin, so this survives a `--base=/fable/` deploy
   * unchanged. In development `vite.config.ts` proxies this prefix (and the
   * emoticon/flag/upload paths the API returns) to `GUESTBOOK_ORIGIN`.
   *
   * An absolute URL works too, but a cross-origin API also needs
   * `credentials: "include"` on the package side and CORS with a concrete
   * origin on the server's — see docs/guestbook-integration.md §8.
   */
  apiBaseUrl: string;
}
