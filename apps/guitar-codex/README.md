...
This is the **host application** of a three-project npm workspace: it composes
two lazy-loaded feature packages, `@guitar-codex/guestbook` and
`@site/demoscene`, and is the only one of the three that is deployed. The map
is in the workspace [`README.md`](../../README.md).

## Run

Always from the **workspace root**, never from this directory — a second
`node_modules` here means a second copy of React, and every hook inside a
linked feature package then throws *"Invalid hook call"*.

```bash
npm install
npm run dev        # http://localhost:5173
npm run build      # apps/guitar-codex/dist/ (pages/ is copied in as the content root)
npm run typecheck  # every workspace
```

## The two feature packages

| | Opened by | Seam | Contract |
|---|---|---|---|
| `@guitar-codex/guestbook` | footer **VI · Guestbook**, at the reserved route `#/guestbook` | `src/components/guestbook/` | [`docs/guestbook-integration.md`](../../docs/guestbook-integration.md) |
| `@site/demoscene` | the **`I`** button in the top bar | `src/components/about/` | [`docs/demoscene-integration.md`](../../docs/demoscene-integration.md) |

Each seam is one dynamic `import()` in a `Lazy*.ts` file, a thin component that
passes props, and a barrel that exports **only** the lazy entry. Neither
package may be imported from startup code — that would put it in the initial
bundle and nothing would visibly break. Turn either off with
`FEATURES.guestbook` / `FEATURES.demoscene` in `src/config.ts`; the guestbook's
API base URL is `GUESTBOOK.apiBaseUrl` beside them, proxied in development by
`vite.config.ts`. Full rules:
[`.claude-memory/17-workspace-and-features.md`](../../.claude-memory/17-workspace-and-features.md).
