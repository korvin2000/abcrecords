# Codex Harmoniae — the production

What `@site/demoscene` actually shows, how it is drawn, and why it is cheap.
The package's **contract** (public API, house rules, adding a language) is in
[`README.md`](README.md); how the host mounts it is in
...
- **Zero runtime dependencies, zero network requests.** No fonts, no image
  files, no audio files: everything is drawn or synthesised at load.
...
To run it on its own, from the workspace root:

```bash
npm run dev:demoscene    # the dev host on :8791
npm run plates           # the pencil-study plate viewer
```

The dev host reads `?locale=xx` off its own URL, so
`http://localhost:8791/?locale=ja` opens it in Japanese — the same handoff the
catalogue makes with a prop.
...
See [`README.md`](README.md) — they belong to the contract, not to these
production notes.
