...
[`react-modular-architecture.md`](../../docs/react-modular-architecture.md) (§n
...
```bash
npm install          # once, at the root — never inside this package
npm run dev:demoscene   # the dev host on :8791 — a stand-in encyclopedia
npm run typecheck       # every workspace
npm run plates          # the pencil-study plate viewer
```

`apps/demoscene-dev-host` is not part of the integration; it exists so this package can
