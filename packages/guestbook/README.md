...
Vom **Wurzelverzeichnis des Workspace** aus, nie aus diesem Verzeichnis
heraus — ein zweites `node_modules` bedeutet ein zweites React:

```bash
npm install
npm run dev:guestbook                    # http://localhost:5174, Proxy auf die echte API
npm run typecheck                        # alle Workspaces
npm run build -w @guitar-codex/guestbook # dist/ — SPA für Notbetrieb / iframe
```
