...
function DocumentRow({ doc }: { doc: DocumentItem }) {
  ...
  if (isAsciiTabUrl(doc.target)) {
    return (
      <button
        type="button"
        onClick={() => openTab({ src: href, label: doc.label, download: filename(doc.target) })}
        className="flex w-full items-center gap-4 border border-gold-600/50 bg-paper-100/70 px-4 py-3 text-left transition-shadow hover:shadow-[0_2px_14px_rgba(138,106,31,0.3)]"
      >
        {body}
        <span className="btn-rpg !px-3 !py-1 !text-[0.65rem]">{t("docs.open")}</span>
  ...
  const body = (
    <>
      ...
      <span className="min-w-0">
        <span className="block truncate font-heading text-sm tracking-wide text-burgundy-700">{t("docs.source")}</span>
        <span className="block truncate text-xs italic text-sepia-600">{url}</span>
      </span>
    </>
  );
  const className = "flex w-full items-center gap-4 border border-gold-600/50 bg-paper-100/70 px-4 py-3 text-left no-underline transition-shadow hover:shadow-[0_2px_14px_rgba(138,106,31,0.3)]";
  return isAsciiTabUrl(url) ? (
    <button type="button" onClick={() => openTab({ src: href, label: filename(url), download: filename(url) })} className={className}>
      {body}
    </button>
  ) : (
