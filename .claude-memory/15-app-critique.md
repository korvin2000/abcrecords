...
- ✅ **[fixed 2026-08-28]** ~~*(scale)* Prefetch is linear → ~1249 serial idle JSON fetches for the full legacy index.~~ At 736 entries the dossier crawl was doing exactly that on **every** visit — `DOSSIER.warmOnIdle` is on so the herald can say "on this day", so every reader paid 736 requests and ~0.7 MB to populate four fields per entry. `config.ts` had already named the escape hatch ("a precomputed digest in `pages/` plugged in behind `factsStore`'s loader — no caller changes"); that is now `vite/facts-digest.ts` → `/facts-<lang>.json`, **one 41–54 KB request (≈15–18 KB gzipped)**, built in memory in dev and emitted as a static asset on build. `factsStore.fill()` tries the digest and falls back to the old crawl when a language has none, and both paths end in `factsFrom`, so the projection cannot drift. **736 requests → 0.**
- 🟡 *(scale)* The digest is derived from `pages/` but never written into it, so it cannot go stale in the repository — but a **static host serving `dist/` will serve a digest frozen at build time**. Content changes need a rebuild (which they already do, since `pages/` is copied into `dist/`).
...
- 🟡 **Only standard & drop-D tunings map to pitch.** `detectTuning` now reads a declared `Tuning: DADGBE` / `Stimmung : E A D G B E` / "Tune the 6th string to D", and an *open* tuning it cannot voice (`abmsaudz` is DGDGBE) deliberately returns `unknown` rather than the wrong drop-D it used to claim — honest, but the notes are still unvoiced. Adding a `custom` kind means a new `TuningKind`, `tab.tuning.custom` in all ten dictionaries, and real open strings in `asciiTabPlayback`.
