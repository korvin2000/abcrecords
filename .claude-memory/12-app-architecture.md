...
> ✅ **Catalogue v2 — code complete (2026-07-31).** Format, data, data layer,
> search and codex are all v2 ([`docs/Catalog-Index.md`](../docs/Catalog-Index.md)
> + [`11`](11-index-json.md); Steps 4, 5, 6 landed). Only **Step 7** of
> [`the plan`](../docs/proposals/Plan_Catalog-v2-index-ids-localized-names-codex-split.md)
> remains: `npm run lint:content` and Vitest specs — the repo still has **no
> test or lint script**, so every invariant is currently unenforced.
...
- ~~`index.json` `country` is free text~~ — **resolved in v2 (Step 4).**
  `index.json` carries lowercase ISO alpha-2, the loader uppercases it once at
  the boundary, and one `countryName(iso, locale)` localizes it everywhere.
  `COUNTRY_TEXT_TO_ISO` / `countryDisplay` / `resolveCountry` /
  `resolveCountryCode` are gone; `.bio.json` no longer carries `country` at all.
