...
| `src/components/fx/ClefSpinner.tsx` · `MusicalDrift.tsx` | The two ornaments. `ClefSpinner` = the 𝄞 that replaced the search loupe, turning 360° about its vertical axis under `perspective(600px)`; the glyph is **always drawn** (it is the icon), only the turn is budgeted. `MusicalDrift` = musical glyphs rising behind the page: N absolutely-positioned spans, **zero JS per frame**, `transform`+`opacity` only, inside a `contain`ed `pointer-events:none` layer; count follows the budget × viewport, `paused` stops the clock while a codex covers it. |
