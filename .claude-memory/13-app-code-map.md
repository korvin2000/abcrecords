...
| `src/index.css` | **The running order, and nothing else** — the font imports, `@import "tailwindcss"`, and fourteen `@import "./styles/*.css"` lines. Since the sheet is unlayered, that list *is* the cascade; the file says so and names the two orderings that matter (`tokens` first, `chrome`'s `btn-rpg` above everything that narrows it). |
| `src/styles/*.css` | The stylesheet itself, split by concern (2 528 lines in one file → 47–318 each): `tokens` · `base` · `chrome` (parchment, gold frame, `btn-rpg`) · `topbar` · `home` (frontispiece, page band, catalogue grid) · `search` · `herald` · `footer` · `codex` · `article` · `figures` (incl. `.fx-curl`) · `blocks` (the BioMD `:::` constructs) · `viewers` (audio pill, ASCII tab) · `effects`. **Still unlayered → beats Tailwind utilities (footgun).** |
