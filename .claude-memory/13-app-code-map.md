...
App.tsx   ─ single orchestrator: catalog load, search criteria, facts crawl gate, hash route, audio toggles, body scroll-lock, codex mount
  ...
  ├─ Dossier facts        dossier/{facts.ts, digest.ts, factsStore.ts, useFacts.ts}  (cross-entry *.bio.json metadata)
...
| `src/lib/dossier/digest.ts` | **The fast path (2026-08-28).** `loadDigest(lang)` fetches `/facts-<lang>.json` — the whole catalogue's searchable metadata in one request — and `factsFromRow` turns a positional `[forename, surname, born, died]` row into the same `EntryFacts` the crawl produces (both go through `factsFrom`, so they cannot drift). A missing digest is a normal `null`, not an error: `factsStore.fill()` then falls back to the per-entry crawl. Built by `vite/facts-digest.ts`. |
